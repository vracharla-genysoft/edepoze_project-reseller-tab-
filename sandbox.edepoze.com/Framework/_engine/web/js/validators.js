var Validators = function() {
            ...
            case 'regularexpression':
                switch (val.mode.toUpperCase()) {
                    ...
                    case 'XOR':
                        var n = 0;
                        for (cid in val.cids)
                            if (this.check(val.cids[cid], valtype, vid)) n++;
                        flag = (n == 1);
                        break;
                }
                break;
    ...
    this._sortValidators = function() {
        var keys = vals = new Array(),
            bind = this;
        for (var i in this.validators) keys[keys.length] = i;
        keys.sort(function(a, b) {
            return ((typeof(bind.validators[a].order) == 'undefined') ? 0 : bind.validators[a].order) - ((typeof(bind.validators[b].order) == 'undefined') ? 0 : bind.validators[b].order);
        });
