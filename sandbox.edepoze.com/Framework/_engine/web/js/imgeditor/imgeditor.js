var ImageEditor = function() {
    ...
    this.initialize = function(options) {
        ...
        controls.addEvent(area, 'selectstart', function() {
            return false;
        });
        ...
        controls.makeDraggable(crop, this.options.editorID, null, function() {
            bind.redrawShadow();
        });
        ...
        controls.addEvent(document.body, 'mouseup', function() {
            bind.isMove = false;
        });
        ...
        var drag = function(snap) {
            var y = parseInt(snap.style.top),
                x = parseInt(snap.style.left);
        ...
        var limit = {
            'x': [size.left + 2, size.right - 2],
            'y': [size.top + 2, size.bottom - 2]
        };
        var limitX = {
                'x': limit.x
            },
        ...
        var transformImage = function(e) {
            ...
            if (code == undefined || code == 0 || code == 13) {
                var angle = parseFloat(controls.$(bind.options.angleID).value),
                    scale = parseFloat(controls.$(bind.options.zoomID).value);
    ...
    this.redrawShadow = function() {
        ...
        if (!bx) bx = 0;
        if (!by) by = 0;
        var cx = parseInt(crop.style.left),
            cy = parseInt(crop.style.top);
        top.style.top = '0px';
        top.style.left = '0px';
    ...
    this.transform = function(angle, scale, dx, dy) {
        ...
        scale = scale / 100;
        var cx = this.image.attrs.x + this.image.attrs.width / 2,
            cy = this.image.attrs.y + this.image.attrs.height / 2;
    ...
    this.getBox = function() {
        var cx = this.image.attrs.width / 2,
            cy = this.image.attrs.height / 2;
        var box = {},
            x1, x2, y1, y2, angle = this.options.angle;
    ...
    this.getCoordinates = function() {
        return {
            'x': this.image.attrs.x,
            'y': this.image.attrs.y
        };
    };
    ...
    this.getCanvasSize = function() {
        return {
            'width': this.paper.canvas.width.baseVal.value,
            'height': this.paper.canvas.height.baseVal.value
        };
    };
};
...
var ImageEditorManager = function() {
    ...
    this.show = function() {
        if (controls.$('imgEditorFade')) controls.fade('imgEditorFade', true, 0.5);
            ...
            var fade = controls.$('fade_' + this.id);
            if (!fade) {
    ...
    this.setup = function(url, width, height, options) {
        ...
        var el = controls.$(options.zoomID),
            coor;
        el.value = 100;
        ...
        var rotate = function(target) {
            var pos = controls.getPosition(target),
                coor = controls.getCoordinates(target.parentNode),
                angle = Math.round(360 * (pos.x - coor.left) / (coor.width - 10));
        ...
        var zoom = function(target) {
            var pos = controls.getPosition(target),
                coor = controls.getCoordinates(target.parentNode),
                scale = Math.round(199 * (pos.x - coor.left) / (coor.width - 10) + 1);
        ...
        controls.makeDraggable(controls.$('slider_rotate_' + this.id).firstChild, null, null, rotate, null, {
            x: [coor.left, coor.right - 10],
            y: [coor.top - 11, coor.top - 11]
        });
        coor = controls.getCoordinates('slider_zoom_' + this.id);
        controls.makeDraggable(controls.$('slider_zoom_' + this.id).firstChild, null, null, zoom, null, {
            x: [coor.left, coor.right - 10],
            y: [coor.top - 11, coor.top - 11]
        });
    };
    ...
    this._setSliderValue = function(el, value, minValue, maxValue) {
        ...
        var coor = controls.getCoordinates(el),
            mix = Math.round(value * (coor.width - 10) / (maxValue - minValue) + coor.left);
        el = controls.$(el).firstChild;
        controls.setPosition(el, {
            x: mix,
            y: coor.top - 11
        });
    }
