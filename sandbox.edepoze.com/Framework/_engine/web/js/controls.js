var Controls = function() {
            ...
            if (a != null) {
                ...
                switch (operator) {
                    ...
                    case '^=':
                        flag = (a.indexOf(b) == 0);
                    case '$=':
                        flag = (a.indexOf(b) == (a.length - b.length));
                }
    ...
    this.query = function(el, selector) {
        ...
        if (typeof el.querySelectorAll == 'undefined') { // for IE7 downto IE4; will not find hidden elements!!!
            ...
            if (el != document.body) {
                if (!el.id) {
                    needClearID = true;
                    el.id = '_search_target_' + String(Math.random()).substr(2)
                }
    ...
    this.getStyle = function(el, property) {
        el = this.$(el);
        if (el.currentStyle) style = el.currentStyle[property.replace(/-\D/g, function(match) {
    ...
    this.getPagePosition = function(event) {
        return {
            x: event.pageX || event.clientX + document.scrollLeft,
            y: event.pageY || event.clientY + document.scrollTop
        };
    };
    ...
    this.getWindowSize = function() {
        if (window.opera || typeof navigator.taintEnabled == 'undefined') return {
            x: window.innerWidth,
    ...
    this.getSize = function(el) {
        el = this.$(el);
        return (this.isBody(el)) ? this.getWindowSize() : {
            x: el.offsetWidth,
            y: el.offsetHeight
        };
    };
    ...
    this.getScrollSize = function(el) {
        el = this.$(el);
        return (this.isBody(el)) ? this.getWindowScrollSize() : {
            x: el.scrollWidth,
            y: el.scrollHeight
        };
    };
    ...
    this.getScroll = function(el) {
        el = this.$(el);
        return (this.isBody(el)) ? this.getWindowScroll() : {
            x: el.scrollLeft,
            y: el.scrollTop
        };
    };
    ...
    this.getOffsets = function(el) {
        ...
        if (el.getBoundingClientRect) {
            var bound = el.getBoundingClientRect(),
                ...
                htmlScroll = this.getScroll(html),
                elemScrolls = this.getScrolls(el),
                elemScroll = this.getScroll(el),
                isFixed = (this.getStyle(el, 'position') == 'fixed');
            return {
                x: parseInt(bound.left) + elemScrolls.x - elemScroll.x + ((isFixed) ? 0 : htmlScroll.x) - html.clientLeft,
                y: parseInt(bound.top) + elemScrolls.y - elemScroll.y + ((isFixed) ? 0 : htmlScroll.y) - html.clientTop
            };
        }
        var sel = el,
            position = {
    ...
    this.getPosition = function(el, relative) {
        el = this.$(el);
        if (this.isBody(el)) return {
            x: 0,
            y: 0
        };
        var offset = this.getOffsets(el),
            scroll = this.getScrolls(el);
        var position = {
            x: offset.x - scroll.x,
            y: offset.y - scroll.y
        };
        var relativePosition = (relative && (relative = this.$(relative))) ? this.getPosition(relative) : {
            x: 0,
            y: 0
        };
        return {
    ...
    this.getCoordinates = function(el, relative) {
        ...
        var position = this.getPosition(el, relative),
    ...
    this.setPosition = function(el, pos) {
        el = this.$(el);
        var position = {
            left: pos.x - parseInt(this.getStyle(el, 'margin-left')),
            top: pos.y - parseInt(this.getStyle(el, 'margin-top'))
        }
        var parent = el.parentNode;
    ...
    this.centre = function(el, overflow) {
        ...
        var scroll = this.getWindowScroll();
        var xx = (winSize.x - size.x) / 2 + scroll.x,
    ...
    this.makeDraggable = function(el, container, fnStart, fnDrag, fnStop, limit) {
        ...
        var fnMouseMove = function(e) {
            ...
            var x = pos.x - sx,
                y = pos.y - sy;
            if (container) {
                var maxX, maxY, minX, minY, sizeContainer = bind.getSize(container),
                    size = bind.getSize(el),
                    pos = bind.getPosition(container);
                minX = pos.x;
                minY = pos.y;
        ...
        var fnMouseDown = function(e) {
            e = e || event;
            var pos = bind.getClientPosition(e),
                cpos = bind.getPosition(el);
            sx = pos.x - cpos.x;
            sy = pos.y - cpos.y;
